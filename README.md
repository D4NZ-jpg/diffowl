...
> [!NOTE]
> The GitHub Action and local CLI use the same Review engine and typed Review outcome contract. The GitHub-hosted path adds durable Git state and coherent GitHub publication; the local and explicit self-hosted paths use filesystem state.
...
`self-hosted-state-directory` is the explicit filesystem persistence input for self-hosted Action runs. A trusted same-repository self-hosted run must set it to a durable directory that survives separate workflow invocations. GitHub-hosted runs ignore filesystem state and always select the dedicated base-repository Git ref. Adapter selection happens before review and never falls back automatically. Do not place a self-hosted state directory under pull-request-controlled content or publish it as an artifact. The Action outputs the typed `outcome`, plus `run-id` and safe `run-metadata` when persistence succeeds. Missing, corrupt, or unwritable required state produces a `configuration_failure` with setup guidance rather than a stateless result.
...
The Action writes a compact job summary and returns the Publication result with confirmed effects separately through the `publication` output, so publication delivery never enters Review engine semantics. The summary identifies Review readiness, Review outcome, the reviewed revision, Finding and Verification counts, coverage limits, Publication result, and links to the pull-request review and workflow logs when available. Publication is `complete`, `not_attempted`, `refused`, or `incomplete`. Confirmed partial effects remain in the Publication result for reconciliation, and a refused or incomplete required publication fails the Action without replacing the Review outcome. The publisher uses the separate SHA-bound, bounded, data-only capability class and cannot execute pull-request code. A default GitHub-hosted run without `GITHUB_TOKEN` returns a preflight `configuration_failure`. Missing `contents: write` fails durable Git state with setup guidance. If GitHub refuses publication because `pull-requests: write` is unavailable, the Publication result fails with GitHub's accepted-permissions guidance; the Action does not treat partial delivery as V1 success. Programmatic engine embeddings without a GitHub publisher record publication as `not_attempted`.
...
`createV1ReleaseReportCard` also accepts the required threat-test and release-integration evidence. `evaluateV1ReleaseGate` refuses V1 when any required threat lacks passing evidence, or when installation metadata, generated bundles, representative workflows, README guidance, the author request/discussion path, targeted human review, or coherent GitHub publication is missing or partial.
